...
# Answers
## Import part of code reviews
I believe an extremely important part of code reviews is looking for edge cases that the dev may have missed. Correct functionality should be a given by the time you enter code review, but that extra set of eyes can help catch small errors or bad assumptions that a single person can make. Another very important part of code review is ensuring the style of the code matches the style put down for the rest of the project.

## How could Docker help ACME
Docker containers serve three very important purposes. First they keep the processes inside the container from affecting other processes, so if a single container crashes it won't bring down any other containers. Secondly they provide an incredibly easy way to scale up any parts of the application, for instance if ACME didn't realize their ordering system would become incredibly busy they can scale up that part of the system by adding on more containers for that part of the application to new machines. Assuming the product is built to scale horizontally of course. Third, they allow for easy setup of the exact environment that will be used in production. Building a product on Windows and then deploying to Linux can be an exercise in frustration, but by doing development in a container representation of our production environmen we can be sure that there won't be environment issues and code development and deployment will be much more smooth.
